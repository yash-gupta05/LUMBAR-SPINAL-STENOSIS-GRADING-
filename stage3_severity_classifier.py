...
#  -----------------------
#  1. `build_sample_list` now accepts `cache`, `best_slice_map`, and
#     a pre-computed `keypoint_map` so it never calls load_dicom_volume,
#     select_best_slice, or predict_keypoints in a hot loop.
#
#  2. `keypoint_map` is built once via `build_keypoint_map()` which
#     runs Stage-2 inference over all series and stores results.
#
#  3. `SeverityDataset.__getitem__` reads from `cache` instead of
#     calling `load_dicom_volume`.
#
#  4. num_workers=0 for the same reason as Stages 1 & 2.
# ============================================================
