...
#  -----------------------
#  1. `VolumeCache` (built in config_and_utils) is pre-loaded once
#     before training and passed into every Dataset and helper.
#     No DICOM I/O happens inside __getitem__ or during validation.
#
#  2. `build_slice_gt` accepts an `inst_map` (pre-built instance→index
#     lookup) to avoid re-scanning DICOM folders in a hot loop.
#
#  3. `SliceSelectorDataset.__getitem__` reads from the cache with a
#     cheap float16→float32 cast instead of hitting the filesystem.
#
#  4. `select_best_slice` likewise accepts a cache so inference in
#     later stages never re-reads DICOMs already in memory.
# ============================================================
